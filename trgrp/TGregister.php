...
function sendemail_verify($name, $email, $verify_token){
    ...
    $email_template = "
        ...
        <br>
        <a href = 'http://localhost/tour2/web_project/trgrp/verify_email.php?token=$verify_token'> Click me </a>
    ";
...
if(isset($_POST['register_btn'])){
    ...
    // Email exists or not
    $check_email_query = "SELECT email FROM trgrp WHERE email='$email' LIMIT 1";
    $check_email_query_run = mysqli_query($connection, $check_email_query);

    if(mysqli_num_rows($check_email_query_run) > 0){
        $_SESSION['status'] = "Email already exists";
        echo "<script> window.location.href='TGsignup.php'; </script>";
    }
    else{
        // Insert User Data
        $query = "INSERT INTO trgrp(name, mobile, email, password,address,verify_token) VALUES ('$name', '$phone', '$email', '$password','$address','$verify_token' )";
